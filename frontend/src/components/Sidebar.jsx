...
export default function Sidebar({ open, onClose, activeKey, onNavigate, onOpenRequest }) {
  ...
  return (
    <>
      ...
      >
        {/* ── Header: Logo + Brand ─────────────────────── */}
        <div className={`${isCollapsed ? 'px-3 pt-4 pb-3' : 'px-5 pt-6 pb-4'} transition-all`} style={{ borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
          <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'justify-between'} mb-${isCollapsed ? '3' : '5'}`}>
            <div className="flex items-center gap-3">
              {branding?.website_logo ? (
                <img src={getImageUrl(branding.website_logo, 'branding')} alt="Logo" className="w-9 h-9 object-contain rounded-xl shrink-0" />
              ) : (
              ...
              {!isCollapsed && (
                <div>
                  <p className="text-white font-black text-base leading-tight">{branding?.website_name || 'Bonus'}</p>
            ...
            {!isCollapsed && (
              <button
                ...
                className="hidden md:flex w-7 h-7 rounded-lg items-center justify-center text-[#9CA3AF] hover:text-white hover:bg-white/10 transition-colors shrink-0"
              >
            ...
            {!isCollapsed && (
              <button
                onClick={onClose}
                className="md:hidden w-7 h-7 rounded-lg flex items-center justify-center text-[#9CA3AF] hover:text-white hover:bg-white/10 transition-colors shrink-0"
              >
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          {/* User card */}
          {isCollapsed ? (
            // Collapsed: show just avatar centered, click to expand
            <div className="flex flex-col items-center gap-2">
              <div
                onClick={() => setProfileOpen(true)}
                className="w-9 h-9 rounded-xl flex items-center justify-center font-bold text-[#1F2937] text-sm shadow-md cursor-pointer overflow-hidden hover:ring-2 hover:ring-[#A3E635]/50 transition-all"
                style={{ background: 'linear-gradient(135deg, #A3E635, #65a30d)' }}
                title={user?.full_name || 'Profile'}
              >
                {user?.profile_image ? (
                  <img src={`/uploads/profiles/${user.profile_image}`} alt="Profile" className="w-full h-full object-cover" />
                ) : initials}
              </div>
              {/* Expand button */}
              <button
                onClick={() => setIsCollapsed(false)}
                className="hidden md:flex w-7 h-7 rounded-lg items-center justify-center text-[#6B7280] hover:text-white hover:bg-white/10 transition-colors"
                title="ขยาย Sidebar"
              >
            ...
            >
              {/* Avatar */}
              <div className="w-9 h-9 rounded-xl flex items-center justify-center font-bold text-[#1F2937] text-sm shadow-md shrink-0 overflow-hidden"
                style={{ background: 'linear-gradient(135deg, #A3E635, #65a30d)' }}>
                {user?.profile_image ? (
                  <img src={`/uploads/profiles/${user.profile_image}`} alt="Profile" className="w-full h-full object-cover" />
                ) : initials}
              ...
              <div className="min-w-0 flex-1">
                <p className="text-white font-bold text-sm truncate leading-tight">{user?.full_name || 'พนักงานช่าง'}</p>
                <div className="flex items-center flex-wrap gap-1 mt-1">
                  {userRoles.map((r, i) => {
                    const label = r === 'super_admin' ? 'Super Admin'
                      : r === 'admin'          ? 'แอดมิน'
                      : r === 'ma_technician'  ? 'ช่าง MA'
                      : r === 'technician' || r === 'office_technician' ? 'ช่าง Office'
                      : r === 'contractor_office' ? 'รับเหมาติดตั้ง'
                      : r === 'contractor_ma'  ? 'รับเหมา MA'
                      : r === 'sales'          ? 'เซล' : r;
                    return (
                      <span key={i} className="text-[9px] font-bold text-[#1F2937] bg-[#A3E635] rounded px-1.5 py-0.5 leading-none">{label}</span>
                    );
                  })}
                  {teamName && <span className="text-[10px] text-[#9CA3AF] truncate ml-0.5">{teamName}</span>}
                </div>
              </div>
              <div className="w-2.5 h-2.5 rounded-full shrink-0 bg-[#A3E635]" style={{ boxShadow: '0 0 8px rgba(163,230,53,0.6)' }} />
            </div>
          )}
        </div>

        {/* ── Navigation ───────────────────────────────── */}
        <nav className={`flex-1 overflow-y-auto py-3 space-y-4 min-h-0 custom-scrollbar ${isCollapsed ? 'px-2' : 'px-3'}`}>
                  ...
                  return (
                    <div key={item.key} className="space-y-0.5">
                      <button
                        ...
                        className={`w-full flex items-center gap-3 rounded-xl transition-all duration-200 relative group overflow-hidden ${
                      ...
                      >
                        ...
                        <div
                          className={`flex items-center justify-center shrink-0 rounded-lg transition-all duration-200 ${
                            ...
                            (isActive || isSubActive) ? 'text-[#A3E635]' : 'text-[#6B7280] group-hover:text-[#D1D5DB]'
                          }`}
                          style={(isActive || isSubActive) ? { background: 'rgba(163,230,53,0.15)' } : { background: 'transparent' }}
                        ...
                        {!isCollapsed && (
                          <span className={`text-sm flex-1 transition-colors duration-200 whitespace-nowrap overflow-hidden ${
                            (isActive || isSubActive) ? 'font-bold text-white' : 'font-medium text-[#9CA3AF] group-hover:text-[#E5E7EB]'
                          }`}>
                            {item.label}
                          </span>
                        )}

                        {/* Arrow / expand — only when expanded */}
                        {!isCollapsed && (
                          <div>
                          {hasSub ? (
                            ...
                            <svg className="w-3 h-3 text-[#A3E635]" fill="currentColor" viewBox="0 0 8 8">
        ...
        {/* ── Footer: Logout ───────────────────────────── */}
        <div className={`py-3 ${isCollapsed ? 'px-2' : 'px-3'}`} style={{ borderTop: '1px solid rgba(255,255,255,0.07)' }}>
          <button
            type="button"
            onClick={(e) => { e.preventDefault(); logout(); }}
            className={`w-full flex items-center gap-3 py-2.5 rounded-xl transition-all text-left group ${
          ...
          >
            <div className="w-8 h-8 rounded-lg bg-red-500/15 flex items-center justify-center group-hover:bg-red-500/25 transition-colors shrink-0">
              <LogoutIcon />
            </div>
            {!isCollapsed && (
              <span className="text-sm font-semibold text-red-400 group-hover:text-red-300 transition-colors">ออกจากระบบ</span>
            )}
          </button>

          {/* Version */}
          {!isCollapsed && (
            <p className="text-center text-[10px] mt-3" style={{ color: 'rgba(163,230,53,0.35)' }}>Bonus v2.0 · AIS Platform</p>
          )}
        </div>
      </aside>
