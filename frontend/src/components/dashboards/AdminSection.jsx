...
/** Open Dispatch with tab + queue (+ optional date) deep-link */
function dispatchPath({ tab = 'office', queue, date } = {}) {
  const params = new URLSearchParams();
  if (tab) params.set('tab', tab);
  if (queue) params.set('queue', queue);
  if (date) params.set('date', date);
  return `/dispatch-dashboard?${params.toString()}`;
}
...
export default function AdminSection() {
  ...
  const fetchData = () => {
    api.get('/stats/admin-dashboard?refresh=1')
  ...
  const incomplete = Number(s.incompleteData || 0);
  const postponedUn = Number(s.postponedUnassigned || 0);
  const officeToday = Number(s.officeAssignedToday || 0);
  const maToday = Number(s.maAssignedToday || 0);
    ...
    <div className="space-y-6 animate-fade-in-up">
      ...
      <div className="flex items-center justify-between gap-3 pb-3 border-b border-[#E5E7EB]">
        <div className="flex items-center gap-3 min-w-0">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-[#A3E635] to-[#65a30d] flex items-center justify-center shadow-md shadow-lime-500/20 shrink-0">
            <span className="text-[#1F2937] text-sm">📋</span>
          </div>
          <div className="min-w-0">
            <h2 className="text-[#1F2937] font-extrabold text-lg leading-tight">คิวจ่ายงาน</h2>
            <p className="text-xs text-[#6B7280] font-medium truncate">มอบหมายงานติดตั้งและ MA — แตะการ์ดเพื่อเปิดคิว</p>
          </div>
        </div>
        <button
          type="button"
          onClick={() => navigate(dispatchPath({ tab: 'office' }))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Left: Queues + Shortcuts */}
        <div className="lg:col-span-2 flex flex-col gap-6">

          {/* Primary assignment queues */}
          <div>
            <div className="flex items-center gap-2.5 mb-4">
              <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-rose-500 to-pink-600 flex items-center justify-center shadow-md">
                <span className="text-white text-sm">⚠️</span>
              </div>
              <h3 className="text-[#1F2937] font-bold text-base">งานที่ต้องจัดการ</h3>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <StatCard
                title="ยังไม่มอบหมาย"
                ...
                urgent={unassigned > 0}
                onClick={() => navigate(dispatchPath({
                  tab: (s.officeUnassigned || 0) >= (s.maUnassigned || 0) ? 'office' : 'ma',
                  queue: 'unassigned',
                }))}
              />
              <StatCard
              ...
              <div className="rounded-2xl border border-[#E5E7EB] bg-[#F9FAFB] p-4 flex flex-col justify-center gap-2">
                ...
                <button
                  type="button"
                  onClick={() => navigate(dispatchPath({ tab: 'office', queue: 'unassigned' }))}
                  className="flex items-center justify-between text-sm font-bold text-[#1F2937] hover:text-[#65a30d] transition-colors"
                >
                  <span>🏢 ติดตั้งรอมอบหมาย</span>
                  <span className="tabular-nums">{Number(s.officeUnassigned || 0)}</span>
                </button>
                <button
                  type="button"
                  onClick={() => navigate(dispatchPath({ tab: 'ma', queue: 'unassigned' }))}
                  className="flex items-center justify-between text-sm font-bold text-[#1F2937] hover:text-[#65a30d] transition-colors"
                >
                  <span>🛠️ MA รอมอบหมาย</span>
                  <span className="tabular-nums">{Number(s.maUnassigned || 0)}</span>
                </button>
              </div>
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center gap-2.5 mb-4">
              ...
              <h3 className="text-[#1F2937] font-bold text-base">มอบหมายแล้ววันนี้</h3>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <StatCard
                title="งานติดตั้งวันนี้"
                value={officeToday}
                suffix="งาน"
                gradient="from-[#374151] to-[#1F2937]"
                icon="🏢"
                shadow="shadow-slate-500/20"
                onClick={() => navigate(dispatchPath({ tab: 'office', queue: 'assigned', date: todayISO() }))}
              />
              <StatCard
                title="งาน MA วันนี้"
                value={maToday}
                ...
                onClick={() => navigate(dispatchPath({ tab: 'ma', queue: 'assigned', date: todayISO() }))}
              />
            </div>
          </div>

          {/* Quick Shortcuts — assignment first */}
          <div className="bg-white rounded-2xl p-5 border border-[#E5E7EB]"
            ...
            <div className="flex items-center gap-2.5 mb-4">
              ...
              <h3 className="text-[#1F2937] font-bold text-base">ทางลัดจ่ายงาน</h3>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              <ShortcutBtn icon="🏢" label="จ่ายงานติดตั้ง" sublabel="มอบหมาย Office"
                onClick={() => navigate(dispatchPath({ tab: 'office' }))}
                gradient="from-emerald-500 to-teal-500" shadow="shadow-emerald-500/25" />
              <ShortcutBtn icon="🛠️" label="จ่ายงาน MA" sublabel="มอบหมายช่าง MA"
                onClick={() => navigate(dispatchPath({ tab: 'ma' }))}
                gradient="from-[#65a30d] to-[#A3E635]" shadow="shadow-lime-500/25" />
              <ShortcutBtn icon="📭" label="คิวยังไม่มอบหมาย" sublabel="งานที่รอทีม/ช่าง"
                onClick={() => navigate(dispatchPath({ tab: 'office', queue: 'unassigned' }))}
              ...
              <ShortcutBtn icon="🎒" label="กระเป๋าช่าง" sublabel="ดูอุปกรณ์ของช่าง"
                ...
                gradient="from-teal-500 to-cyan-600" shadow="shadow-teal-500/25" />
              <ShortcutBtn icon="📦" label="คลังสินค้า" sublabel="รับ-จ่ายสินค้า"
                onClick={() => navigate('/inventory')}
                gradient="from-[#374151] to-[#1F2937]" shadow="shadow-slate-500/25" />
            </div>
          </div>
        </div>

        {/* Right: Announcements + Online */}
        <div className="lg:col-span-1">
                  ...
                  <div key={ann.id}
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-amber-700 mt-1 line-clamp-3 whitespace-pre-wrap">{ann.content || ann.message}</p>
          ...
          <div className="bg-white rounded-3xl border border-[#E5E7EB] overflow-hidden flex flex-col h-[420px] shadow-sm mt-6">
