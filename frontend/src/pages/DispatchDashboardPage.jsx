...
export default function DispatchDashboardPage() {
  ...
  const handleCancelCompletion = (job) => {
    requestConfirm('ยืนยันการยกเลิกจบงาน', `คุณต้องการยกเลิกการจบงานของ ${job.access_no} ใช่หรือไม่? ระบบจะคืนอุปกรณ์เข้ากระเป๋าช่าง และลบยอดผลงานโดยอัตโนมัติ`, async () => {
      try {
        await axios.put(`/dispatch/jobs/${job.id}/cancel-completion`);
        fetchJobs();
        showNotification('ยกเลิกการจบงานและคืนอุปกรณ์เรียบร้อยแล้ว', 'success');
      } catch (err) {
        showNotification(err.response?.data?.error || 'ไม่สามารถยกเลิกการจบงานได้', 'error');
      }
    }, true);
  };
