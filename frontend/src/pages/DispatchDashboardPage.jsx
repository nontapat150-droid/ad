        ...
        <div className="bg-white md:rounded-2xl md:max-w-lg md:w-full md:max-h-[90vh] rounded-t-3xl max-h-[90vh] flex flex-col shadow-2xl animate-[slideUp_0.25s_ease-out]">
          ...
          <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4" style={{ scrollbarWidth: 'thin' }}>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                { label: 'เบอร์โทร', value: job.phone, link: job.phone ? `tel:${job.phone}` : null },
                { label: 'ทีมช่าง', value: job.team_name || (isAdmin ? 'ยังไม่ระบุ' : '-') },
                ...
                { label: 'เวลา', value: job.plan_arrival_time ? (typeof job.plan_arrival_time === 'string' ? job.plan_arrival_time.substring(0,5) : '-') : '-' },
                ...
                <div key={label} className="bg-[#F9FAFB] rounded-xl p-3 border border-[#E5E7EB]">
                  ...
                  {link ? (
                    <a href={link} className="text-sm font-semibold text-emerald-600 hover:underline break-all">{value || '-'}</a>
                  ) : (
        ...
        <header className="bg-white border-b border-[#E5E7EB] shrink-0" style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
          ...
          <div className="flex items-center gap-3 px-4 py-3">
            ...
            <div className="flex items-center gap-1.5">
              ...
              {isAdmin && <>
                <button onClick={() => setIsImportModalOpen(true)} className="hidden sm:flex items-center gap-1.5 px-3 py-2 bg-[#F3F4F6] hover:bg-[#E5E7EB] text-[#374151] border border-[#E5E7EB] rounded-xl text-xs font-semibold transition-colors">
