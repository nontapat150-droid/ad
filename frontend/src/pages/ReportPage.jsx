                  ...
                  {reports.map((report) => (
                    <div key={report.id} className="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
                      <div className="flex flex-col sm:flex-row gap-4">
                        {/* Image Thumbnail */}
                        {report.image_path && (
                          <div className="shrink-0 w-full sm:w-32 h-32 rounded-xl overflow-hidden border border-slate-100 bg-slate-50 relative group cursor-pointer" onClick={() => setViewerPhoto(`/api/uploads/${report.image_path}`)}>
                            <img src={`/api/uploads/${report.image_path}`} alt="Report" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
