...
// ── POST /api/oil/recalculate-debug ──────────────────────────────
router.post('/recalculate-debug', async (req, res) => {
  try {
    const conn = await pool.getConnection();
    let logs = [];
    ...
    for (const record of records) {
      const plate = record.license_plate ? record.license_plate.replace(/\s+/g, '').toLowerCase() : 'unknown';
      let distance = 0;
      const currentMileage = parseFloat(String(record.mileage || '').replace(/,/g, '')) || 0;
      ...
      const totalPrice = parseFloat(String(record.total_price || '').replace(/,/g, '')) || 0;
      const bahtPerKm = distance > 0 ? (totalPrice / distance).toFixed(2) : 0;
      batchValues.push([distance, parseFloat(bahtPerKm) || 0, record.id]);
    }

    logs.push(`Total records to update: ${batchValues.length}`);
    ...
    for (let i = 0; i < batchValues.length; i += CHUNK_SIZE) {
      ...
      try {
        ...
        logs.push(`Chunk ${i} updated successfully (size ${chunk.length})`);
      } catch(e) {
        logs.push(`SQL Error in chunk ${i}: ${e.message}`);
        logs.push(`SQL: ${sql.substring(0, 200)}...`);
      }
    }

    conn.release();
    res.json({ success: true, logs, sample_values: batchValues.slice(0, 10) });
  } catch (err) {
